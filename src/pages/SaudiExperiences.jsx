...
const SaudiExperiences = () => {
    const [searchQuery, setSearchQuery] = useState('');
    const [currentPage, setCurrentPage] = useState(1);
    const packagesPerPage = 20;

    // Filter packages for 'Discover Saudi Arabia' category
    const filteredPackages = useMemo(() => {
        return packages.filter(pkg =>
            pkg.subType === 'Visit Saudi' &&
            pkg.ksaCategory === 'Discover Saudi Arabia' &&
    ...
    const Hero = (
        <SubPageHero
            title={<>Saudi <span className="text-primary">Experiences</span></>}
            subtitle="Discover diverse landscapes, rich culture, and unforgettable moments in Saudi Arabia. From pristine coasts to mountain retreats."
            ...
            <div className="lg:col-span-8">
                ...
                {filteredPackages.length === 0 && (
                    <div className="text-center py-20">
                        <p className="text-xl text-gray-500">No experiences found matching your search.</p>
                    </div>
                )}
            </div>
...
export default SaudiExperiences;
